...
import { ChevronDown, Grid, Heart, List, Search, ShoppingBag, SlidersHorizontal, Sparkles, Star, X } from 'lucide-react';
      ...
      <div className="container mx-auto px-4 py-4 md:py-8 mt-16 md:mt-20 mb-20 md:mb-0">
        ...
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            <h1 className="text-3xl font-bold">{t('products.newArrivals.newArrivals')}</h1>
          </div>
          <p className="text-muted-foreground">
            {t('products.newArrivals.freshStylesJustLandedBeThe')}
          </p>
        </motion.div>
        ...
        >
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div className="relative flex-1">
              ...
              <Input
                placeholder={t('products.newArrivals.searchNewArrivals')}
                value={localSearchQuery}
                onChange={(e) => onSearchChange(e.target.value)}
                className="pl-10 pr-10 h-12 transition-all duration-200 focus:border-info-500 focus:ring-1 focus:ring-info-500"
              />
              {localSearchQuery && (
                <Button
                  variant="ghost"
            ...
            <Select value={sortBy} onValueChange={setSortBy}>
              <SelectTrigger className="w-full lg:w-48 h-12">
                <SelectValue placeholder={t('products.newArrivals.sortBy')} />
              </SelectTrigger>
              <SelectContent>
            ...
            <div className="hidden lg:flex border rounded-lg p-1 bg-muted">
              <Button
                ...
                className="px-4 py-2 transition-all duration-200 hover:bg-info-50"
              >
                <Grid size={16} className="mr-2" />
                {t('products.newArrivals.grid')}
              </Button>
              <Button
            ...
            <Button
              ...
              className="hidden lg:flex h-12 px-4 bg-card border-2 border-info-100 hover:border-info-300 hover:bg-info-50 transition-all duration-200 font-medium"
            >
              <SlidersHorizontal size={16} className="mr-2 text-info-600" />
              {t('products.newArrivals.filters')}
              <ChevronDown 
                size={16} 
                className={`ml-2 transition-transform text-info-600 ${showFilters ? 'rotate-180' : ''}`} 
              />
            </Button>
          </div>
          ...
          <div className="flex lg:hidden gap-3">
            ...
            <div className="flex-1">
              <Sheet>
                <SheetTrigger asChild>
                  <Button 
                    variant="outline" 
                    className="w-full h-12 bg-card border-2 border-info-100 hover:border-info-300 hover:bg-info-50 transition-all duration-200 font-medium"
                  >
                    <SlidersHorizontal size={18} className="mr-2 text-info-600" />
                    {t('products.newArrivals.filters')}
                  </Button>
                </SheetTrigger>
                <SheetContent side="left" className="w-80 p-0 bg-card">
                  <SheetHeader className="p-6 pb-0 text-left">
                    <SheetTitle className="text-left text-xl font-semibold text-foreground">{t('products.newArrivals.filters')}</SheetTitle>
                  </SheetHeader>
                  <div className="p-6">
                    <MobileFilterContent 
            ...
            <div className="flex-1">
              <div className="flex h-12 border-2 border-info-100 rounded-lg p-1 bg-card">
                <Button
                  variant={viewMode === 'grid' ? 'default' : 'ghost'}
                  size="sm"
                  onClick={() => setViewMode('grid')}
                  className={`flex-1 h-full transition-all duration-200 ${
                    viewMode === 'grid' 
                      ...
                      : 'hover:bg-info-50 text-muted-foreground'
                  }`}
                >
        ...
        <div className="hidden lg:flex gap-8">
          ...
          <div className="flex-1">
            <div className="mb-4 flex items-center justify-between">
              <p className="text-muted-foreground">
                {t('common.showingOfItems', { shown: products.length, total: pagination.total, items: t('products.newArrivals.newArrivals2') })}
              </p>
              {hasActiveFilters && (
                <div className="flex items-center gap-2">
                  <span className="text-sm text-muted-foreground">{t('products.newArrivals.activeFilters')}</span>
                  {searchQuery && (
                  ...
                  {minRating > 0 && (
                    <Badge variant="secondary" className="gap-1">
                      {minRating}{t('products.newArrivals.stars')}
                      <X 
                        size={12} 
                        className="cursor-pointer" 
            ...
            <motion.div
              ...
              className={`grid ${
                viewMode === 'grid' 
                  ? 'grid-cols-2 gap-x-5 gap-y-10 md:grid-cols-3 lg:gap-x-7 xl:grid-cols-4 xl:gap-y-12' 
                  : 'grid-cols-1 gap-6'
              }`}
            >
              <AnimatePresence>
                {loading ? (
                  [...Array(6)].map((_, index) => (
                    <div key={index} className="animate-pulse border rounded-lg h-96 bg-muted/30" />
                  ))
                      ...
                      {viewMode === 'grid' ? (
                        ...
                        <div className="space-y-3">
                          <Link href={`/products/${product.slug}`}>
                            ...
                            <div className="relative bg-card rounded-lg overflow-hidden aspect-square">
                              ...
                              <button
                                ...
                                className="absolute top-3 right-3 w-9 h-9 bg-card rounded-full flex items-center justify-center shadow-md hover:scale-110 transition-transform duration-200 z-10"
                              >
                                <Heart 
                                  size={18} 
                          ...
                          <div className="space-y-2 px-1">
                            <Link href={`/products/${product.slug}`}>
                              {/* Category */}
                              <p className="text-xs sm:text-sm text-subtle-foreground font-medium uppercase tracking-wide mb-1">
                                {product.category?.name || t('products.newArrivals.newArrival')}
                              </p>

                              {/* Product Name */}
                              <h3 className="text-sm sm:text-base font-medium text-foreground line-clamp-2 mb-2">
                                {product.name}
                              </h3>
                            </Link>
                            ...
                            <div className="flex items-center justify-between gap-2 sm:gap-3">
                              <div className="flex flex-col">
                                <span className="text-sm sm:text-base font-price font-semibold text-foreground">
                                  {formatPrice(product.price)}
                                </span>
                                {product?.comparePrice !== 0 && product?.comparePrice !== undefined && product?.comparePrice > product?.price && (
                                  <span className="text-xs sm:text-sm font-caption text-subtle-foreground line-through">
                              ...
                              <Button
                                ...
                                size="sm"
                                className="bg-gradient-to-r from-primary-500 to-secondary-500 hover:from-primary-600 hover:to-secondary-600 text-white px-2 xs:px-3 sm:px-4 py-1.5 xs:py-2 text-xs xs:text-xs sm:text-sm font-semibold rounded-md shadow-sm flex-shrink-0 flex items-center gap-1.5"
                              >
                                <ShoppingBag className="h-3 w-3 xs:h-3.5 xs:w-3.5 sm:h-4 sm:w-4" />
                                {t('products.newArrivals.addToCart')}
                              </Button>
                            </div>
                          </div>
                        </div>
            ...
            {!loading && products.length === 0 && (
              ...
              >
                ...
                <h3 className="text-xl font-semibold mb-2">{t('products.newArrivals.noNewArrivalsFound')}</h3>
                <p className="text-muted-foreground mb-4">
                  {t('products.newArrivals.tryAdjustingYourSearchCriteriaOr')}
                </p>
                <Button onClick={clearFilters}>{t('products.newArrivals.clearAllFilters')}</Button>
              </motion.div>
            )}
        ...
        <div className="lg:hidden bg-card">
          ...
          {hasActiveFilters && (
            <div className="mb-4">
              <div className="flex flex-wrap gap-2">
                <span className="text-sm text-muted-foreground">{t('products.newArrivals.filters2')}</span>
                {searchQuery && (
                  <Badge variant="secondary" className="gap-1">
                    "{searchQuery}"
                    <X 
                    ...
                    {viewMode === 'grid' ? (
                      ...
                      <div className="space-y-2">
                        <Link href={`/products/${product.slug}`}>
                          ...
                          <div className="relative bg-card rounded-lg overflow-hidden aspect-square">
                            ...
                            <button
                              ...
                              className="absolute top-2 right-2 w-7 h-7 bg-card rounded-full flex items-center justify-center shadow-md z-10"
                            >
                              <Heart 
                                size={14} 
                                className={wishlistItems.some(item => item.id === product._id) ? 'fill-current text-destructive-500' : 'text-muted-foreground'} 
                        ...
                        <div className="space-y-1.5 px-0.5">
                          <Link href={`/products/${product.slug}`}>
                            {/* Category */}
                            <p className="text-[10px] xs:text-xs text-subtle-foreground font-medium uppercase tracking-wide mb-1">
                              {product.category?.name || t('products.newArrivals.newArrival')}
                            </p>

                            {/* Product Name */}
                            <h3 className="text-xs xs:text-sm font-medium text-foreground line-clamp-2 mb-1.5 leading-tight">
                              {product.name}
                            </h3>
                          </Link>
                          ...
                          <div className="flex items-center justify-between gap-1.5">
                            <div className="flex flex-col min-w-0">
                              <span className="text-xs xs:text-sm font-price font-semibold text-foreground truncate">
                                {formatPrice(product.price)}
                              </span>
                              {product?.comparePrice !== 0 && product?.comparePrice !== undefined && product?.comparePrice > product?.price && (
                                <span className="text-[10px] xs:text-xs font-caption text-subtle-foreground line-through truncate">
                                  {formatPrice(product.comparePrice)}
                                </span>
                              )}
                            </div>
                            ...
                            >
                              <ShoppingBag className="h-3 w-3 xs:h-3.5 xs:w-3.5 sm:h-4 sm:w-4 mr-1" />
                              {t('products.newArrivals.add')}
                            </Button>
                          </div>
                        </div>
                      </div>
          ...
          {!loading && products.length === 0 && (
            ...
            >
              ...
              <h3 className="text-lg font-semibold mb-2">{t('products.newArrivals.noNewArrivalsFound')}</h3>
              <p className="text-muted-foreground text-sm mb-4">
                {t('products.newArrivals.tryAdjustingYourSearchOrFilters')}
              </p>
              <Button onClick={clearFilters} size="sm">{t('products.newArrivals.clearAllFilters')}</Button>
            </motion.div>
          )}
...
const MobileFilterContent = ({ 
  ...
  <div className="space-y-6">
    <div className="flex items-center justify-between">
      <h3 className="font-semibold">{t('products.newArrivals.filters')}</h3>
      <Button variant="ghost" size="sm" onClick={clearFilters}>
        {t('products.newArrivals.clearAll')}
      </Button>
    </div>

    {/* Category Filter */}
    <div className="space-y-3">
      <h4 className="font-medium">{t('products.newArrivals.category')}</h4>
    ...
    {/* Price Range */}
    <div className="space-y-3">
      <h4 className="font-medium">{t('products.newArrivals.priceRange')}</h4>
      <Slider
        value={priceRange}
    ...
    {/* Color Filter */}
    <div className="space-y-3">
      <h4 className="font-medium">{t('products.newArrivals.color')}</h4>
    ...
    {/* Rating Filter */}
    <div className="space-y-3">
      <h4 className="font-medium">{t('products.newArrivals.minimumRating')}</h4>
      <div className="space-y-2">
        {[4, 3, 2, 1].map(rating => (
          <div key={rating} className="flex items-center space-x-2">
            ...
            <label htmlFor={`mobile-rating-${rating}`} className="flex items-center space-x-1 text-sm cursor-pointer">
              ...
              <span>{t('products.newArrivals.up')}</span>
            </label>
          </div>
        ))}
      </div>
